...
CSS = '''
/* ===================== SOL RAY -> UST CUBUK =====================
   Ray, 880px ustunde header.nav'i 240px kenar cubuguna ceviriyordu.
   Yatay cubuga cevirirken rayin SUSLERI de olculup tek tek kapatildi:
     .brand-mark           1141. satirda 112px'e cikiyordu -> 28px
     .nav-links::before    2px dikey ray cizgisi           -> kapali
     .nav-links a::after   11x34 radyal nokta              -> yatay alt cizgi
     .nav-right::after     content:"© ÆRAY" ray dipnotu    -> kapali
*/
:root{ --sb:0px; --bar:60px }
@media (min-width:880px){
  ...
  header.nav{
    ...
    border-right:none; border-bottom:1px solid rgba(120,210,255,.22);
    background:linear-gradient(180deg,rgba(18,52,100,.94),rgba(9,28,56,.9));
  ...
  .nav-inner{
    ...
    height:var(--bar); max-width:1500px; margin:0 auto; gap:0 clamp(14px,1.8vw,26px);
    padding:0 max(18px,2.4vw); overflow:visible;
  }

  /* marka — gec media query'yi kesin kuralla ez */
  header.nav .brand{ flex:0 0 auto; flex-direction:row; align-items:center;
                     gap:10px; margin:0; padding:0; border:0 }
  header.nav .brand-mark,
  header.nav .brand-mark svg{ width:28px !important; height:28px !important;
                              animation:none !important; filter:none }
  header.nav .brand-mark svg g{ stroke-width:8 }
  header.nav .brand-wm{ flex-direction:column; align-items:flex-start; gap:1px }
  header.nav .brand-wm b{ font-size:12.5px !important; letter-spacing:.07em !important }
  header.nav .brand-wm i{ font-size:8px !important; letter-spacing:.15em !important }

  /* menu — rayin dikey cizgisi ve noktalari kapali */
  .nav-links{ flex:1 1 auto; flex-direction:row; align-items:center; flex-wrap:nowrap;
              gap:0 clamp(10px,1.3vw,20px); margin:0; padding:0; border:0; overflow:visible }
  .nav-links::before,.nav-links::after{ content:none !important; display:none !important }
  .nav-links a{ padding:5px 0; margin:0; border:0; font-size:12.5px; white-space:nowrap }
  .nav-links a::before{ content:none !important; display:none !important }
  .nav-links a::after{
    content:"" !important; position:absolute !important;
    left:0 !important; right:0 !important; top:auto !important; bottom:-3px !important;
    width:auto !important; height:2px !important; border-radius:2px;
    background:#5ec2ff !important;
    transform:scaleX(0); transform-origin:left; transition:transform .26s ease;
  }
  .nav-links a:hover::after,.nav-links a.active::after{ transform:scaleX(1) }

  /* sag blok — ray dipnotu kalksin, dikeyden yataya */
  .nav-right{ flex:0 0 auto; flex-direction:row; align-items:center; gap:12px;
              width:auto !important; height:auto !important;
              margin:0 0 0 auto !important; padding:0 !important; border:0 !important }
  .nav-right::after{ content:none !important; display:none !important }
  .lang{ margin:0 }
